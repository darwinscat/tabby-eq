...
public:
    ...
    // Width the badge needs at a given height: pads + cat square + gap + the "TabbyEQ" wordmark.
    int preferredWidth (int height) const
    {
        const float h = (float) height;
        return (int) std::ceil (kPadL + catSize (h) + kGap + tabby::brand::textWidth (wordFont (h), kWord) + kPadR);
    }
    ...
    void paint (juce::Graphics& g) override
    {
        ...
        // --- content: the two logos — the cat mark + the "TabbyEQ" wordmark, vertically centred ---
        ...
        const float baseline = content.getCentreY() + (wf.getAscent() - wf.getDescent()) * 0.5f;
        g.setFont (wf);
        g.setColour (hover ? juce::Colours::white : juce::Colour (0xffeef0f6));
        g.drawSingleLineText (kWord, juce::roundToInt (catArea.getRight() + kGap), juce::roundToInt (baseline));
    }

private:
    static float catSize (float h) { return h * 0.74f; }
    static float wordH   (float h) { return h * 0.42f; }    // "TabbyEQ"

    juce::Font wordFont (float h) const { return tabby::brand::wordmarkFont (wordmarkTypeface, wordH (h)); }

    bool hover = false;
    static constexpr float kPadL = 11.0f, kGap = 10.0f, kPadR = 14.0f;
